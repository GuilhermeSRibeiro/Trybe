...
SELECT MAX(SALARY) - MIN(SALARY) AS 'Diferença entre o maior e o menor salário'
FROM employees;
...
SELECT SUM(SALARY) AS 'Total de salários' FROM employees;
...
SELECT JOB_ID, SUM(SALARY) AS 'Total de salários de pessoas programadoras'
FROM employees
GROUP BY JOB_ID
HAVING JOB_ID = 'IT_PROG';

-- 9. Escreva uma query que exiba em ordem decrescente a média salarial de todos os cargos, exceto das pessoas programadoras (IT_PROG).
SELECT JOB_ID, AVG(SALARY) AS `Média salarial dos cargos, exceto pessoas programadoras`
FROM employees
GROUP BY JOB_ID
HAVING JOB_ID <> 'IT_PROG'
...
SELECT EMPLOYEE_ID AS 'Id', FIRST_NAME AS 'Nome', MONTH(HIRE_DATE) AS 'Mês em que foi contratado'
FROM employees;
...
SELECT FIRST_NAME AS 'Nome', LAST_NAME AS 'Sobrenome', DATEDIFF(CURRENT_DATE(),HIRE_DATE) AS 'Dias que trabalha na empresa' FROM employees;
